...
let
  ...
  hash-dir = path: lib.pipe (builtins.readDir path) [
    attrsToList
    (map ({ name, value }: if value == "regular" then hash-file name else if value == "directory" then hash-dir name else throw ("unhandled hash-dir filetype arg at path " + name + ": " + value)))
